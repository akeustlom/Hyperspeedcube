...
puzzles:add{
	...
	build = function(self)
		...
		self:mark_piece(F(1) & R(1) & U(1), 't', "T-Center")
		self:mark_piece(F(1) & ~side_cut(F, R).region & ~top_cut(F, U).region & top_cut(F, D).region, 't')
		self:mark_piece(U(1) & ~L(1) & ~top_cut(F,D).region & side_cut(F,L).region, 'x', "X-Center")
